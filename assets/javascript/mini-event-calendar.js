...
(function($) {
  ...
  $.fn.miniEventCalendar = $.fn.MEC = function(options) {
    ...
    function populateCalendar(month, year) {
      ...
      if (ldate.getDate() === 1 && dt.getDay() != 1) tbody.append(last_prev_month_days(dt.getDay()));
      while (ldate.getMonth() === month) {
        ...
        if (ldate.getMonth() != month)
          for (var i = 1; i < buffer_days; i++) tbody.append(date_tpl(true, i));
      }
    }
    ...
    function last_prev_month_days(day) {
      ...
      var last_days = "";
      for (var i = day; i > 0; i--) last_days += date_tpl(true, prev_month[prev_month.length - i]);
      return last_days;
    }
